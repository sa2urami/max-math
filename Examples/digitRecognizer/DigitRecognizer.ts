...
for (let i = 0; i < EpochCount; i++) {
    ...
    for (let j = 0; j < BatchSize; j++) {
        ...
        // @ts-ignore
        const { data: pixelsData } = await getPixels(join('Examples/digitRecognizer/train/', file))
        // @ts-ignore
        ...
        if (answer == argmax(m.layers[m.layers.length - 1].value.toArray())) yes++
        n++
    }
    console.log(yes / n + ' ' + (n - yes) / n)
    yes = 0
    n = 0
}
